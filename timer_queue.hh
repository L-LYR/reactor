...
class TimerQueue : public ChannelCallback {
  ...
  private:
    ...
    using TimerList = std::set<TimerListEntry>;  // sorted and 1:n one time point for many timers.
    ...
    auto reset_repeat_timers(const std::vector<TimerListEntry>& expired, const Timestamp& now) -> void;
