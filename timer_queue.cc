...
TimerQueue::TimerQueue(EventLoop* p_loop)
    : mp_loop(p_loop),
      mp_add_timer_wrapper(new AddTimerWrapper(this)),
...
auto TimerQueue::create_timerfd() -> void {
    ...
    if (m_timerfd < 0) {
        std::cout << "error in timerfd_create(), errno: " << errno << std::endl;
    }
}
...
auto TimerQueue::reset_timerfd(const Timestamp& ts) -> void {
    ...
    auto nanoseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(microseconds - seconds);
    ...
    if (::timerfd_settime(m_timerfd, 0, &new_timerspec, NULL) < 0) {
        std::cout << "error in timerfd_settime(), errno: " << errno << std::endl;
    }
}

auto TimerQueue::reset_repeat_timers(const std::vector<TimerListEntry>& expired, const Timestamp& now) -> void {
...
auto TimerQueue::get_expired_timers(const Timestamp& now) -> std::vector<TimerListEntry> {
    std::vector<TimerListEntry> expired_timers;
    auto it = std::lower_bound(m_timer_list.begin(), m_timer_list.end(), TimerListEntry(now, timer_ptr_for_find));
...
auto TimerQueue::insert(Timer* p_timer) -> bool {
    ...
    if (!ret.second) {
        std::cout << "errnr in insert()" << std::endl;
    }
...
auto TimerQueue::remove(const TimerListEntry& entry) -> void {
    ...
    if (it != m_timer_list.end()) {
        ...
        std::cout << "remove an inexistent timer" << std::endl;
    }
}