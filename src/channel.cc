...
Channel::Channel(EventLoop *event_loop, int sockfd)
    : m_sockfd(sockfd), m_events(0), m_revents(0), m_state(is_new),
      m_callback(nullptr), mp_event_loop(event_loop) {}

auto Channel::set_callback(ChannelCallback *callback) -> void {
  m_callback = callback;
}
...
auto Channel::is_writing() -> bool { return m_events & EPOLLOUT; }

auto Channel::update() -> void { mp_event_loop->update(this); }
