...
Buffer::Buffer(size_t initial_size)
    : m_buffer(buffer_prepend_size + initial_size),
      m_read_index(buffer_prepend_size), m_write_index(buffer_prepend_size) {}

auto Buffer::readable_bytes() -> size_t { return m_write_index - m_read_index; }

auto Buffer::writable_bytes() -> size_t {
...
auto Buffer::has_written(size_t length) -> void { m_write_index += length; }
...
auto Buffer::make_space(size_t length) -> void {
  if (writable_bytes() + prependable_bytes() < length + buffer_prepend_size) {
    ...
    std::copy(begin() + m_read_index, begin() + m_write_index,
